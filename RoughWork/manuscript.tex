...
The model's skill is reported through a few statistical measures of the the total water level $\eta$ and the residual $\delta$. First, the mean absolute error ($MAE$), defined as
\begin{equation}
MAE = \text{mean}\left(\left| M - O \right|\right),
\end{equation}
...
\hline 
Location       & $MAE$ (cm) & $\gamma^2$ & $WS$   & $MAE$ (cm) & $\gamma^2$ & $WS$          \\
\hline 
Point Atkinson & 10.5       &   0.012    & 0.994  & 6.2        &  0.0646    & 0.979         \\
Victoria       &  8.6       &   0.032    & 0.992  & 6.1        &  0.0851    & 0.970         \\
Patricia Bay   &  9.1       &   0.024    & 0.992  & 6.9        &  0.0704    & 0.969         \\
Campbell River & 13.1       &   0.041    & 0.990  & 5.8        &  0.0790    & 0.961         \\
\hline 
\end{tabular}
\caption{Evaluation of model performance for the Feb 2006 simulation. Mean absoulte error (MAE), $\gamma^2$, and the Willmott skill score ($WS$) are provided for water level predictions ($\eta$) and resdiuals ($\delta$) during the period displayed in Figure \ref{fig:feb2006}.}
