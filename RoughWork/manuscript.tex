...
Finally, an assessment of the importance in including tide-surge interactions is presented next. 

\begin{figure}
\centering
\includegraphics[scale=0.6]{Figures/feb2006_factors.pdf}
\caption{Comparison of the modelled residual for three simulations: A simulation with all forcing (blue), a simulation without atmospheric forcing (green), and a simulation without the sea surface height anomaly. }
\label{fig:factors}
\end{figure}
%not sure about the colours, but different line styles is really hard to make out. 
%Maybe only need the surge plot?
