...
In general, there is also very good agreement between the observed and modelled residuals (Figure \ref{fig:feb2006}). The most noticeable difference is {\color{red} a broader peak in the model's surge curve, leading to a delay of a few ours in timing of the peak surge for most of the stations (Table \ref{tab:peak}). Note that the surge manifests as a broad peak in both the modelled and observed residual leading to a high probability that large residuals coincide with the high tide.} As such, it is of minor consequence that the timing of the observed and modelled maximum residual do not match perfectly. 
